  ...
  return (
    <div className="relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          <h2 className="font-display font-bold text-2xl text-white">Internal Marks Register</h2>
          <p className="text-xs text-neutral-400 mt-1">Upload raw evaluation marks for student midterms and finals. Released marks convert to letter grades instantly.</p>
        </div>
      </div>

      {/* Selections Controls Card */}
      <div className="glass-card border border-neutral-800 rounded-xl p-4 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
        {/* Subject */}
        <div className="flex items-center gap-2 bg-neutral-950 border border-neutral-850 rounded px-2 text-xs text-white">
          ...
          >
            {subjects.map(sub => (
              <option key={sub.subjectId} value={sub.subjectId} className="bg-neutral-950 text-white">
                {sub.subjectName}
              </option>
            ))}
          </select>
        ...
        <div className="flex items-center gap-2 bg-neutral-950 border border-neutral-850 rounded px-2.5 text-xs text-white">
          ...
          <input
            ...
            className="bg-transparent text-white font-bold w-16 focus:outline-none text-center"
          />
        </div>
      </div>
            ...
            <tbody className="divide-y divide-neutral-900 text-neutral-300">
              {roster.length > 0 ? (
                roster.map(student => {
                  ...
                  return (
                    <tr key={student.studentId} className="hover:bg-neutral-900/30 transition">
                      <td className="px-4 py-3 flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-neutral-800 border border-neutral-700 flex items-center justify-center font-bold text-blue-400">
                          {student.name.charAt(0)}
                        </div>
                        <span className="font-semibold text-white">{student.name}</span>
                      </td>
                      <td className="px-4 py-3 font-mono">{student.rollNo}</td>
                      <td className="px-4 py-3 text-neutral-500">{maxMarks}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className={`w-24 px-2 py-1 bg-neutral-950 border text-white rounded text-xs font-semibold focus:outline-none ${
                              isErr ? "border-rose-500 ring-1 ring-rose-500/25" : "border-neutral-850"
                            }`}
                          />
                          {isErr && (
                ...
                <tr>
                  <td colSpan={4} className="text-center py-10 text-neutral-500 font-mono text-xs">
