...
export default function HodReportCenter() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setError(res.message || "Failed to load department report.");
      }
    } catch (err: any) {
      console.error(err);
      setError(err.message || "Error fetching department report.");
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-6 text-left">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-text-primary">Department Report Desk</h2>
            <p className="text-xs text-text-secondary">
              Review and export registers scoped strictly to your active department.
            </p>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            >
              <option value="attendance">Department Attendance</option>
              <option value="results">Academic Results</option>
              <option value="fees">Student Fee Dues</option>
              <option value="lms">LMS Student Workloads</option>
              <option value="mentorship">Mentorship Records</option>
              <option value="opportunities">Opportunity participation</option>
              <option value="teaching">Teaching Planner execution</option>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-650 dark:bg-indigo-600 dark:hover:bg-indigo-750 text-white rounded-xl text-xs font-semibold bg-accent-blue hover:bg-accent-blue/95 transition-all shadow-md shadow-accent-blue/10 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shrink-0"
            >
              <Download size={14} />
              <span>Export</span>
            </button>
        ...
        {/* Filter Panel (Scoped to HOD role) */}
        <FilterPanel onApplyFilters={handleApplyFilters} role="hod" />
        ...
        <div className="bg-surface border border-border-subtle rounded-2xl overflow-hidden shadow-sm">
          {loading ? (
            <div className="py-24 flex flex-col items-center justify-center text-text-muted">
              ...
              <span className="font-mono text-xs">Assembling department ledger...</span>
            </div>
          ) : error ? (
            ...
            <div className="w-full">
              ...
              <div className="px-5 py-4 border-b border-border-subtle bg-surface-elevated/40 flex justify-between items-center">
                <span className="text-xs font-bold text-text-primary uppercase tracking-wider">
                  {reportData.title || "Department Ledger"}
                </span>
